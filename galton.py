...
pg.display.set_caption("Galton Simulation")
...
for i in range(10):
    ...
    for j in range(WIDTH // step + 2):
        ...
        if i == 9:
            create_segment((peg_x, peg_y + 100), (peg_x, HEIGHT - 20), segment_thickness, space, 'darkslategray')
        peg_x += step
    peg_y += 0.5 * step

platforms = (L1, L2), (L2, L3), (L3, L4), (R1, R2), (R2, R3), (R3, R4)
for platform in platforms:
    create_segment(*platform, segment_thickness, space, 'darkolivegreen')   # funnel
create_segment(B1, B2, 10, space, 'darkslategray')                          # floor
...
while True:
    ...
    if pg.mouse.get_pressed()[0]:
        ball_moment = pymunk.moment_for_circle(ball_mass, 0, ball_radius)
        ball_body = pymunk.Body(ball_mass, ball_moment)
        ball_body.position = pg.mouse.get_pos()
        ball_shape = pymunk.Circle(ball_body, ball_radius)
        ball_shape.elasticity = 0.0
        ball_shape.friction = 0.25
        space.add(ball_body, ball_shape)
        balls.append( ([randrange(256) for i in range(3)], ball_body) )
